Vytvoř nový programovací jazyk s názvem HydraX v Pythonu. Přidej příkazy:

echo (výpis textu) místo print
dump (vytáhne data)
log (zaznamenává logy)
load (načte data ze souboru nebo knihovny)
clone (duplikuje obsah)
run (spustí program nebo soubor)
hydrax soubor.hdrx (spuštění souboru HydraX)
venomx -b build soubor.hdrx (kompilace HydraX souboru)
Přidej minimálně 60 příkazů na:

matematické operace
hashing
šifrování a dešifrování
HydraX soubory budou mít příponu .hdrx.

Přidej 10 knihoven s vlastními názvy:

webi (Flask/FastAPI pro weby)
socket (Python socket)
cookie (Python json)
tokio (Python asyncio)
requests (Python requests)
hashlib (Python hashlib, pro hashing)
dalších 4 knihovny podle potřeby
