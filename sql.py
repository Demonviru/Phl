...
def stop_webcam():
    global webcam_stream_running
    webcam_stream_running = False
    print("Webcam stream stopped.")

# Global flags for controlling stream state
webcam_stream_running = False
...
def webcam_stream(client_socket):
    global webcam_stream_running
    webcam_stream_running = True
    cap = cv2.VideoCapture(0)
    while webcam_stream_running:
        ret, frame = cap.read()
        ...
        try:
            ...
            break
    cap.release()
    webcam_stream_running = False  # Set the flag to False when done
