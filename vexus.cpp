...
std::pair<std::vector<unsigned char>, std::vector<unsigned char>> generatePostQuantumKeyPair() {
    OQS_KEM *kem = OQS_KEM_new(OQS_KEM_alg_default);
    if (kem == NULL) {
        throw std::runtime_error("Failed to initialize post-quantum KEM.");
    }

    std::vector<unsigned char> publicKey(kem->length_public_key);
    std::vector<unsigned char> secretKey(kem->length_secret_key);

    if (OQS_KEM_keypair(kem, publicKey.data(), secretKey.data()) != OQS_SUCCESS) {
        OQS_KEM_free(kem);
        throw std::runtime_error("Failed to generate post-quantum keypair.");
    }
...
int main() {
    ...
    auto [pqPublicKey, pqSecretKey] = generatePostQuantumKeyPair();

    std::cout << "Post-Quantum Keypair Successfully Generated!" << std::endl;
